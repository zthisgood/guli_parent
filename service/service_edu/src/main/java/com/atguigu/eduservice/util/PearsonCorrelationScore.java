...
public class PearsonCorrelationScore {
    ...
    private Map<String, Map<String, Double>> dataset = new HashMap<String, Map<String, Double>>();
...
//        initDataSet();
     ...
     * @param map
     */
    public void addDataMap(String userId, Map<Integer, Integer> map){
        Map<String, Double> map1 = new HashMap<>();
        //遍历 map 重建数据结构
        for (Map.Entry<Integer, Integer> entry: map.entrySet()){
            map1.put(entry.getKey().toString(), Double.parseDouble(entry.getValue().toString()));
        }
        dataset.put(userId, map1);
    }
     ...
     * @return string
     */
    public String getBestMatchUser(String userId){
        double score = 0.0;
        double newScore;
        String user = "";
        for (String user2: this.dataset.keySet()){
            if(user2.equals(userId)){
                continue;
            }
            newScore = this.sim_pearson(userId, user2);
            if (score<newScore){
                score = newScore;
                user = user2;
            }
    ...
    public static void main(String[] args) {
        ...
        pearsonCorrelationScore.initDataSet();
